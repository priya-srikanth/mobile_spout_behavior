...
// Protocol: v2 structured line protocol + richer status/stats
// Adds machine-readable sync pulse events for GUI/session logging
// ============================================================

// -----------------------------
// Pins
// -----------------------------
// Breakout-board / lick-board compatible pins
// Single-side task wiring (left set only), simplified for current breakout-board constraints.
static const uint8_t PIN_SYNC_OUT = 2;
static const uint8_t PIN_SPEAKER  = 3;            // audio PWM out
static const uint8_t PIN_TTL_STARTSTOP = 4;       // session start/stop TTL
static const uint8_t PIN_REWARD_LEFT_SOLENOID  = 5;
static const uint8_t PIN_TTL_TRIAL   = 6;         // trial-start TTL

// SMC02 motion-control lines, using direct button-emulation method validated by debug sketch.
...
static const uint32_t DEFAULT_DOCK_WICK_MS = 500;
...
struct BehaviorConfig {
  ...
  uint32_t dockWickMs = DEFAULT_DOCK_WICK_MS;
...
void setup() {
  ...
  digitalWrite(PIN_TTL_STARTSTOP, LOW);
  ...
  pinMode(PIN_LICK_LEFT_IN, INPUT);
...
void axisStop(Axis& a) {
  ...
  uint32_t t0 = millis();
  while (millis() - t0 < SMC02_STOP_PULSE_MS) {
...
bool moveAxisRelative(Axis& a, float deltaMM) {
  ...
  smcReleaseLine(a.pinCW);
  smcReleaseLine(a.pinCCW);
  delay(MOTOR_DIRECTION_GUARD_MS);

  smcPressLine(drivePin);
  uint32_t t0 = millis();
  while (millis() - t0 < moveMs) {
    handleSerialDuringBlocking();
    serviceCore();
  }
...
bool moveToPositionSafe(const Vec3& target) {
  ...
  uint32_t t0 = millis();
  while (millis() - t0 < LICK_REARM_AFTER_MOTION_MS) {
    handleSerialDuringBlocking();
    serviceCore();
  }
  return true;
...
float effectiveDownwardAngleDegForAzimuth(float azDeg) {
  ...
  for (int i = 0; i < NUM_AZIMUTHS; i++) {
    float a = fabsf(geom.azimuthDeg[i]);
    if (a > maxAbsAz) maxAbsAz = a;
  }

  if (maxAbsAz < 0.001f) {
    return geom.downwardAngleDeg;
  }

  float frac = azDeg / maxAbsAz;
  float offset = -0.5f * geom.headRollDeg * frac;
  return geom.downwardAngleDeg + offset;
}
...
void recomputePosition(uint8_t idx) {
  ...
  float downDeg = effectiveDownwardAngleDegForAzimuth(azDeg);

  float phi = azDeg * PI / 180.0f;
  float theta = downDeg * PI / 180.0f;

  positions[idx].x = mouthOrigin.x + r * cosf(theta) * sinf(phi);
  positions[idx].y = mouthOrigin.y - r * cosf(theta) * cosf(phi);
  positions[idx].z = mouthOrigin.z - r * sinf(theta);
}
...
uint32_t sampleITI() {
  return cfg.dockWickMs + cfg.itiMinMs + random(cfg.itiJitterMs + 1);
}
...
void pulsePin(uint8_t pin, uint32_t durMs) {
  ...
  else if (pin == PIN_TTL_STARTSTOP) pulseEndStartStop = tEnd;
...
void updateTTLPulses() {
  ...
  if (pulseEndStartStop && now >= pulseEndStartStop) {
    digitalWrite(PIN_TTL_STARTSTOP, LOW);
    pulseEndStartStop = 0;
  }
...
void emitPositions() {
  for (int i = 0; i < NUM_POSITIONS; i++) {
    ...
    Serial.println(effectiveDownwardAngleDegForAzimuth(geom.azimuthDeg[positionAzIndex[i]]), 3);
  }
}
...
void emitConfig() {
  ...
  emitConfigKV("task.dock_wick_ms", cfg.dockWickMs);
...
String normalizeLegacySetKey(const String& key) {
  ...
  if (key.equalsIgnoreCase("DOCK_WICK_MS")) return "task.dock_wick_ms";
...
bool handleSet(const String& key, const String& value) {
  ...
  if (key == "task.dock_wick_ms") {
    if (!parseUIntValue(value, utmp)) return false;
    cfg.dockWickMs = utmp; return true;
  }
...
bool handleSerialDuringBlocking() {
  ...
  if (cmd.equalsIgnoreCase("STOP")) {
    ...
    smcAllRelease(axisY);
    smcAllRelease(axisZ);
    pulsePin(PIN_TTL_STARTSTOP, TTL_PULSE_MS);
    emitOK("stop");
    return true;
  }
