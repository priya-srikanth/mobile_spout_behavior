...
//
// v40 changes vs v39 (2026-08-14):
//   Hybrid lick detection for Teensy rigs:
//   1. Keep the v36-style debounced lick_on / lick_off state machine for stability.
//   2. Add a tiny early-confirm onset latch during the raw pressed level so brief real ENL licks
//      can still reset the pre-cue timer even if they release before full debounce.
//   3. No pin-change interrupt path, so one physical press produces one onset latch per press.
//
...
// -----------------------------
#define RIG_2PRAM 0     // shared Teensy on Pavel breakout board, 2-photon room
#define RIG_GB219 1     // dedicated Teensy, behavior room / photometry rig
...
bool lickPressedPrev = false;
bool lickEarlyOnsetConsumedForPress = false;
uint32_t lickPressedSinceMs = 0;
...
void updateLick() {
  ...
  // Hybrid Teensy lick handling:
  // - raw press level can arm one early onset per physical press after a tiny confirm interval,
  //   which helps catch brief ENL violations before full debounce;
  // - lick_on / lick_off events still require the older debounced state transition.
  if (currentState) {
    if (!lickPressedPrev) {
      lickPressedPrev = true;
      lickPressedSinceMs = now;
      lickEarlyOnsetConsumedForPress = false;
    } else if (!lickEarlyOnsetConsumedForPress &&
               lickSensingEnabled &&
               (now - lickPressedSinceMs >= LICK_EARLY_ONSET_CONFIRM_MS) &&
               (now - lastLickOnsetMs >= lickCfg.refractoryMs)) {
      lickOnsetLatched = true;
      lastLickOnsetMs = now;
      lickEarlyOnsetConsumedForPress = true;
    }
  } else {
    lickPressedPrev = false;
    lickPressedSinceMs = 0;
    lickEarlyOnsetConsumedForPress = false;
  }
