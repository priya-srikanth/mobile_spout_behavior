...
static const uint8_t PIN_TTL_STARTSTOP = 9;   // session start/stop TTL to DAQ
...
float effectiveDownwardAngleDegForAzimuth(float azDeg) {
  ...
  for (int i = 0; i < NUM_AZIMUTHS; i++) {
    float a = fabs(geom.azimuthDeg[i]);
    if (a > maxAbsAz) maxAbsAz = a;
  }
  if (maxAbsAz < 0.001f) return geom.downwardAngleDeg;
...
void recomputePosition(uint8_t idx) {
  ...
  positions[idx].x = mouthOrigin.x + r * cos(theta) * sin(phi);
  positions[idx].y = mouthOrigin.y - r * cos(theta) * cos(phi);
  positions[idx].z = mouthOrigin.z - r * sin(theta);
}
...
void emitPositions() {
  for (int i = 0; i < NUM_POSITIONS; i++) {
    ...
    Serial.print(" down_deg="); Serial.println(effectiveDownwardAngleDegForAzimuth(geom.azimuthDeg[positionAzIndex[i]]), 3);
  }
}
...
void updateTTLPulses() {
  ...
  if (pulseEndStartStop && now >= pulseEndStartStop) { digitalWrite(PIN_TTL_STARTSTOP, LOW); pulseEndStartStop = 0; }
}
...
bool handleSet(const String& key, const String& value) {
  ...
  if (key == "task.dock_wick_ms") { if(!parseUIntValue(value,u)) return false; cfg.dockWickMs=u; return true; }
...
void handleSerial() {
  ...
  if (cmd.equalsIgnoreCase("STOP")) { cfg.sessionRunning = false; runState = ST_IDLE; pulsePin(PIN_TTL_STARTSTOP, TTL_PULSE_MS); emitOK("stop"); return; }
  if (cmd.equalsIgnoreCase("HOME")) { homeAllAxes(); return; }
  ...
  if (cmd.equalsIgnoreCase("RESETSESSION")) { resetSessionStats(); resetAdaptiveDistances(); recomputeAllGeneratedPositions(); normalizeSchedulerConfig(); nextSyncAt = millis() + random(syncMinIntervalMs, syncMaxIntervalMs + 1); emitOK("resetsession"); return; }
