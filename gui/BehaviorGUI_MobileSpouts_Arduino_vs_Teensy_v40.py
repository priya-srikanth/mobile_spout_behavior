...
class BaseApp(tk.Tk):
    ...
    def _compute_geometry_positions(self):
        ...
        max_abs_az = max(abs(v) for v in az) if any(abs(v) > 1e-9 for v in az) else 1.0
        out = []
        for i, (r, az_deg, label) in enumerate(position_map):
            frac = az_deg / max_abs_az if max_abs_az > 1e-9 else 0.0
            down_eff = down - 0.5 * head_roll * frac
            theta = math.radians(down_eff)
            phi = math.radians(az_deg)
            x = mouth[0] + r * math.cos(theta) * math.sin(phi)
            y = mouth[1] - r * math.cos(theta) * math.cos(phi)
            z = mouth[2] - r * math.sin(theta)
            out.append({"index": i, "label": label, "dist_mm": r, "az_deg": az_deg, "down_deg": down_eff, "x": x, "y": y, "z": z})
        return mouth, out
