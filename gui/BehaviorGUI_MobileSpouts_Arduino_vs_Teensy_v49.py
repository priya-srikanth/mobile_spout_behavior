...
class SessionLogger:
    ...
    #: Fields whose presence means this row received a trial BODY, not just a trial_start.
    _OUTCOME_FIELDS = ("hit", "miss", "reward_delivered", "lick_in_response_window")

    @classmethod
    def _row_has_outcome(cls, row) -> bool:
        """Did this row ever receive a cue/lick/hit/miss/reward, or is it an empty placeholder?

        The firmware emits ``trial_start`` carrying the PREVIOUS trial's id, so v47's fix -- close the
        open row on every ``trial_start`` -- necessarily opens one placeholder row per trial that only
        ever holds position and timing and never an outcome. Those placeholders DOUBLED trials.csv:
        PS92 2026-08-12 wrote 160 rows for 81 trials, and 563 rows for 283 trials across the session,
        so row counts read as trial counts and the session looked twice its real size.

        Nothing is lost by not writing them. Every consumer already selects on hit XOR miss, and
        ``events.csv`` keeps every ``trial_start`` in full with its own position and timestamp.
        """
        return any(str(row.get(k, "")) not in ("", "0", "False", "false", "None")
                   for k in cls._OUTCOME_FIELDS)
    ...
    def _finalize_trial(self, force=False):
        ...
        if not force and not row.get("trial_id", ""):
            return
        if not self._row_has_outcome(row):
            # placeholder opened by a trial_start that never became a trial -- drop it (v49)
            self._current_trial = None
            return
...
class BaseApp(tk.Tk):
    def __init__(self):
        ...
        self._lick_trace_is_digital = True
    ...
    def _build_task_tab(self):
        ...
        ttk.Label(logic, text=(
            ...
            "Auto-hold after misses: in auto reward modes, hold further rewards after the threshold of consecutive missed rewarded trials; resume immediately on the next detected lick."
    ...
    def _build_lick_tab(self):
        ...
        txt = (
            "Current Teensy and Mega/Zaber firmware use digital lick sensing.\n\n"
            "Suggested starting values:\n"
            "- polarity = -1\n"
            "- refractory = 20 ms\n\n"
            "The live lick trace is a digital state monitor. Threshold / hysteresis / baseline fields are retained "
            "for protocol compatibility with older builds, but they do not drive the current digital lick logic."
        )
    ...
    def _parse_serial_line(self, line: str):
        ...
        if line.startswith("STAT kind=lick"):
            ...
            if not is_digital:
                is_digital = raw in (0.0, 1.0) and baseline in (0.0, 1.0)
            self._lick_trace_is_digital = is_digital
            if is_digital:
                raw_plot = 1.0 if raw >= 0.5 else 0.0
                self.lick_samples.append((time.time(), raw_plot, None, None, lick))
            else:
                try:
    ...
    def _draw_lick_trace(self):
        ...
        if digital_mode:
            def y_of(v):
                return top + (1 - max(0.0, min(1.0, float(v)))) * (bottom - top)
        else:
            def y_of(v):
        ...
        legend = 'digital state=green  lick high=red tint' if digital_mode else 'raw=green  baseline=blue  threshold=red'
        c.create_text(right, top, anchor='ne', text=legend, fill='#555555')
    ...
    def load_motion_coords_from_config_dialog(self):
        ...
        try:
            path = Path(path)
            data = json.loads(path.read_text())
            self._apply_motion_coords_from_config_dict(data, source_path=path)
    ...
    def _apply_motion_coords_from_config_dict(self, cfg, source_path=None):
        cfg = self._migrate_legacy_z_positive_down_cfg(cfg)
        self.safe_z_var.set(cfg.get("safe_z", self.safe_z_var.get()))
        mouth = cfg.get("mouth", [])
        if len(mouth) == 3:
        ...
        if len(dock) == 3:
            self.dock_x_var.set(dock[0]); self.dock_y_var.set(dock[1]); self.dock_z_var.set(dock[2])
        profile_name = str(cfg.get("mouse_profile_name", "") or "").strip()
        if profile_name:
            self.current_mouse_profile_var.set(profile_name)
        elif source_path is not None:
            self._set_current_mouse_profile(source_path)
...
class App(BaseApp):
    ...
    def _refresh_backend_note(self):
        if self.backend_var.get() == "mega_zaber":
            ...
            self.backend_note_var.set(
                "Teensy/SMC02 backend is active. Digital lick sensing and SMC02 timed-motion calibration apply, the COM port should be the Teensy, and no X/Y/Z home switches are assumed. "
